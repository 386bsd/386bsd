/* TODO:crash dump routine, disklabel shit, peel out buffer at low ipl,
	...
	register struct buf *bp;
{
	...
	case RECAL:
		...
		outb(wdc+wd_cyl_lo, du->dk_dd.dk_ncylinders+1);
		outb(wdc+wd_cyl_hi, (du->dk_dd.dk_ncylinders+1)>>8);
		outb(wdc+wd_sdh, WDSD_IBM | (unit << 4) 
...
outb(0x461,0);	/* disable failsafe timer */
	...
	num = (u_long) num * NBPG / du->dk_dd.dk_secsize;
...
pg("xunit %x, nblocks %d, dumplo %d num %d\n", xunit,nblocks,dumplo,num);
	...
	while ((inb(wdc+wd_status) & WDCS_BUSY) && (i-- > 0)) nulldev() ;
	outb(wdc+wd_sdh, WDSD_IBM | (unit << 4));
	outb(wdc+wd_command, WDCC_RESTORE | WD_STEP);
	...
	outb(wdc+wd_cyl_lo, du->dk_dd.dk_ncylinders+1);
	outb(wdc+wd_cyl_hi, (du->dk_dd.dk_ncylinders+1)>>8);
	outb(wdc+wd_sdh, WDSD_IBM | (unit << 4) + du->dk_dd.dk_ntracks-1);
	outb(wdc+wd_seccnt, du->dk_dd.dk_nsectors);
	outb(wdc+wd_command, 0x91);
	while ((stat = inb(wdc+wd_status)) & WDCS_BUSY) nulldev();
	stat = inb(wdc+wd_error);
...
#endif
		CMAP1 = PG_V | PG_KW | ((int)addr&PG_FRAME);
		load_cr3(rcr3());
