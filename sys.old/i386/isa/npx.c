...
npxinit(control) {
	...
	asm("	fnsave %0 " : : "g" (u.u_pcb.pcb_savefpu) );
...
npxdna() {
	...
	if (!(u.u_pcb.pcb_flags & FP_WASUSED)
	    ||(u.u_pcb.pcb_flags & FP_NEEDSRESTORE)) {
		load_cr0(rcr0() & ~CR0_EM);	/* stop emulating */
		asm("	frstor %0 " : : "g" (u.u_pcb.pcb_savefpu) );
		u.u_pcb.pcb_flags |= FP_WASUSED | FP_NEEDSSAVE;
		u.u_pcb.pcb_flags &= ~FP_NEEDSRESTORE;
		npxproc = u.u_procp;
