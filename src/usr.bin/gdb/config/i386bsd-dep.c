...
get_register_offset()
{
	unsigned int offset, useroffset;
	struct user u;	/* XXX */
	unsigned int    flags = (char *) &u.u_pcb.pcb_flags - (char *) &u;

	setregmap(ptrace(PT_READ_U, inferior_pid, (caddr_t)flags, 0));

	/* find the user structure location */
	offset = (char *) &u.u_pcb.pcb_tss.tss_esp0 - (char *) &u;
	useroffset = ptrace(PT_READ_U, inferior_pid, (caddr_t)offset, 0);
	useroffset -= UPAGES * NBPG;

	/* find the start of registers */
	...
	CORE_ADDR addr;
{
	 ...
	 *
	 * Here's the scoop.  KernelPTD contains the physical address
	 * of a page table directory that always maps the kernel.
	 * KernelPTD is in memory that is mapped 1-to-1, so we can
	 * find it easily given its 'virtual' address from ksym_lookup().
	 *
	 * To look up a kernel address, we first convert it to a 1st-level
	 * address and look it up in KernelPTD.  This gives us the physical
	 ...
	 * kernel space, we must look it up using KernelPTD.  If we do have
...
#ifdef NEWVM
	physrd(ksym_lookup("KernelPTD") - KERNOFF, &sbr, sizeof sbr);
	slr = 2 * NPTEPG;			/* XXX temporary */
	printf("KernelPTD %x\n", sbr);
	if (devmem) {
	curproc = ksym_lookup("curproc") - KERNOFF;
	physrd(curproc, &curproc, sizeof curproc);
printf("curproc %x\n", curproc);
	curpcb = curproc;
	curpcb += (unsigned) &p.p_addr - (unsigned) &p;
printf("curpcb %x\n", curpcb);
	/*curpcb -= KERNOFF;
	physrd(curpcb, &curpcb, sizeof curpcb); */
	curpcb = kvread(curpcb);
printf("curpcb %x\n", curpcb);
	/* curpcb = ksym_lookup("curpcb") - KERNOFF;
	physrd(curpcb, &curpcb, sizeof curpcb);*/
	/* kstack = ksym_lookup("kstack");*/
	}/* else {
	curpcb = ksym_lookup("dumppcb") - KERNOFF;
	physrd(curpcb, &curpcb, sizeof curpcb);
	}*/
#else
	...
	if (!devmem) {
		...
		read_pcb(vtophys(ksym_lookup("dumppcb")));
	}
#ifdef NEWVM
	else
	read_pcb(vtophys(curpcb));
#endif

	stack_start = pcb.pcb_ksp - ctob(UPAGES);
	stack_end = pcb.pcb_ksp;
	...
	char *arg;
{
	...
	if (!arg)
		error_no_arg("virtual address");
	if (!kernel_debugging)
		error("not debugging kernel");
	uaddr = (u_int) parse_and_eval_address(arg);
        printf("Physical %x\n", (val = vtophys(uaddr)));
	/*hist = record_latest_value (val);
  	if (hist >= 0)
		printf_filtered ("$%d = ", hist);*/
}

set_kernel_frame_command(arg)
	char *arg;
{
	u_int uaddr;
	int eip, ebp;
	...
	if (!arg)
		error_no_arg("kernel virtual address for new stack frame");
	if (!kernel_debugging)
		error("not debugging kernel");
	uaddr = (u_int) parse_and_eval_address(arg);
	ebp = kvread(uaddr);
	...
	int             from_tty;
{
	...
	if (kernel_debugging) {
		...
		/* find kernel stack */
      		curproc = ksym_lookup("curproc");
printf("curproc@ %x\n", curproc);
      		if (lseek (corechan, curproc, 0) < 0)
			perror_with_name ("seek on core file");
      		if (myread (corechan, &curproc, sizeof (struct proc *)) < 0) 
			perror_with_name ("read from core file");
printf("curproc %x\n", curproc);
		curpcb = curproc;
		curpcb += (unsigned) &p.p_addr - (unsigned) &p;
printf("curpcb@ %x\n", curpcb);
      		if (lseek (corechan, curpcb, 0) < 0)
			perror_with_name ("seek on core file");
      		if (myread (corechan, &curpcb, sizeof (struct pcb *)) < 0) 
			perror_with_name ("read from core file");
printf("curpcb %x\n", curpcb);
		stack_start = curpcb;
		stack_end = curpcb + ctob(UPAGES);
	} else
#endif
	{
		/*
		...
		stack_end =  (unsigned)u.u_kproc.kp_eproc.e_vm.vm_maxsaddr
			+ MAXSSIZ;
		...
		reg_offset = (unsigned) u.u_kproc.kp_proc.p_md.md_regs
				 - (unsigned)(u.u_pcb.pcb_tss.tss_esp0 - UPAGES * NBPG);
#endif
...
\"current\" process context for kernel debugging.");
	...
	add_com ("physical-address", class_obscure, get_physaddr_command,
		 "The virtual address ADDR has its physical address returned.");
	add_com_alias ("vtophys", "physical-address", class_obscure, 0);
	add_com ("kernel-frame", class_obscure, set_kernel_frame_command,
		 "The virtual address ADDR is used as the current stack frame.");
	add_com_alias ("kframe", "kernel-frame", class_obscure, 0);
#endif
}
